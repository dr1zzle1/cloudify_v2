...
export function uploadFile(file, dirId, currentUser) {
  return async (dispatch) => {
    try {
      ...
      const response = await axios.post(
        `https://serene-brushlands-28260.herokuapp.com/api/files/upload`,
        formData,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      ...
      dispatch(addFile(response.data));
      dispatch(setUser({ ...currentUser, usedSpace: currentUser.usedSpace + file.size }));
    } catch (e) {
      alert(e.response.data.message);
    }
  };
...
export async function downloadFile(file) {
  const response = await fetch(
    `https://serene-brushlands-28260.herokuapp.com/api/files/download?id=${file._id}`,
    {
      headers: {
  ...
  return async (dispatch) => {
    try {
      const response = await axios.delete(
        `https://serene-brushlands-28260.herokuapp.com/api/files?id=${file._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
